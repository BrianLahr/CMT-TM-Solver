...
% ============================================================
% SALVAR RESULTADOS PARA TODOS OS MODOS (SEM ALTERAR PROCESSAR_MODOS)
% ============================================================
num_points = 2000;
num_modes = length(found_betas);
result = struct();
result.mode = struct([]);

% --- 0. PREPARAR DADOS GEOMÉTRICOS E CONSTANTES ---
result.geometry.t_interfaces = t; % Posições das interfaces (m)
result.geometry.t_layers = layer_thickness; % Espessuras das camadas (m)
% Extrair índices n_k (Apenas a parte real ou o termo diagonal de Ex)
n_k = zeros(num_layers, 1);
for i = 1:num_layers
    % Para materiais isotrópicos: n_k = sqrt(Epsilon(1,1))
    % Para anisotrópicos/girotrópicos: usaremos a raiz quadrada do termo principal
    n_k(i) = sqrt(layer_perms{i}(1,1));
end
result.geometry.n_k = n_k; % Índices de refração (complexos, se for o caso)

% Constantes Físicas
result.consts.lambda = c_0 * 2 * pi / omega; % Reconstruindo Lambda (m)
result.consts.omega = omega;
result.consts.eps_0 = eps_0;


for m = 1:num_modes
    fprintf("Processando e salvando modo %d...\n", m);
    beta_m = found_betas(m);
    % --- 1. Chamar sua função ORIGINAL (não modificada) ---
    [A_vec, B_vec, gamma_vec, fator_norm, integral_val] = ...
        processar_modos_encontrados(beta_m, num_layers, t, layer_perms, eps_0, omega, k_0);
    % --- 2. Calcular campos deste modo ---
    [x_points, Hy_points, Ex_points, Ez_points] = ...
        calculate_fields(num_layers, t, A_vec, B_vec, gamma_vec, ...
                         layer_perms, omega, eps_0, beta_m, num_points);
    % --- 3. Salvar no struct result.mode(m) ---
    result.mode(m).x_points = x_points;
    result.mode(m).Hy_points = Hy_points;
    result.mode(m).Ex_points = Ex_points;
    result.mode(m).Ez_points = Ez_points;
    result.mode(m).A = A_vec;
    result.mode(m).B = B_vec;
    result.mode(m).gamma = gamma_vec;
    result.mode(m).beta = beta_m;
    result.mode(m).fator_normalizacao = fator_norm;
